import { Ship, Bell, User, BarChart2 } from "lucide-react";
...
export default function Header({ dossiers, showAnalyticsLink = true }: HeaderProps) {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-20 h-14 flex items-center justify-between px-6"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-5">
        <Stat value={dossiers.length} label="Dossiers actifs" color="text-white" />
        <Divider />
        <div className="flex items-center gap-2">
          ...
          <Stat
            value={totalAlertes}
            label="Alertes"
            color={alertesHautes > 0 ? "text-red-400" : "text-amber-400"}
          />
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-300 text-sm">Othmane</span>
        </div>
      </div>
    </header>
  );

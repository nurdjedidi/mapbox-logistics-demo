...
import { Ship, AlertCircle, X } from "lucide-react";
...
export default function LeftPanel({ dossiers, selectedId, onSelect, onClose, isMobile = false }: LeftPanelProps) {
  if (isMobile) {
    return (
      <motion.div
        initial={{ y: "100%" }}
        ...
        className="fixed bottom-0 left-0 right-0 z-30 flex flex-col rounded-t-2xl overflow-hidden"
        style={{ ...mobileGlass, height: "72vh" }}
      >
        <div className="flex items-center justify-between px-4 py-3.5" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          {onClose && (
            <button onClick={onClose} className="text-slate-600 hover:text-slate-300 transition-colors cursor-pointer p-1">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
        <DossierList dossiers={dossiers} selectedId={selectedId} onSelect={onSelect} />
      </motion.div>
    );
  }
...
function DossierList({ dossiers, selectedId, onSelect }: { dossiers: Dossier[]; selectedId: string | null; onSelect: (id: string) => void }) {
  return (
